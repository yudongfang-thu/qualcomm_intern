# QNN 本地文档数据库与 Claude Code / Codex Skill

> 目的：不把 Qualcomm 原始手册镜像上传到 GitHub。到公司机器后现场抓取公开文档、建立本地数据库，再让 Claude Code 或 Codex skill 检索本地数据库回答问题。
...
```text
.codex/skills/qnn-local-docs/SKILL.md
```

两份 skill 都使用同一套本地检索脚本和本地 SQLite 数据库。辅助摘要分别在：

```text
.claude/skills/qnn-local-docs/references/qnn_quick_summary.md
.codex/skills/qnn-local-docs/references/qnn_quick_summary.md
```
...
如果 Codex 没有自动发现仓库内 skill，可以复制到本机 Codex skill 目录：

```bash
mkdir -p ~/.codex/skills
cp -R .codex/skills/qnn-local-docs ~/.codex/skills/
```

然后在 Codex 里这样问：

```text
请使用 $qnn-local-docs 帮我查 QNN 手册：HTP 怎么跑模型，需要量化吗？
```
